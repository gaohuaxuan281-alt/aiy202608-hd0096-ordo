...
export async function requestOpenAIResponse({
  ...
  history: AIMessage[];
  message: string;
  structuredOutput?: StructuredJSONFormat;
}) {
  const runtimeEnv = env as unknown as OpenAIRuntimeEnv;
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      text: structuredOutput
        ? {
            verbosity: module === "ai-tutor" ? "medium" : "low",
            format: {
              type: "json_schema",
        ...
        : { verbosity: module === "ai-tutor" ? "medium" : "low" },
      max_output_tokens: structuredOutput ? 2200 : module === "ai-tutor" ? 1800 : 1400,
      store: false,
      safety_identifier: await createSafetyIdentifier(userId),
    }),
    signal: AbortSignal.timeout(structuredOutput ? 120_000 : 60_000),
  });
